...
describe('usePushRefresh (app do filho)', () => {
  afterEach(() => vi.unstubAllGlobals());

  it('recarrega pedidos, alertas e o contador quando o SW avisa que chegou push', () => {
    const { target, invalidate } = setup();

    target.dispatchEvent(new MessageEvent('message', { data: { type: 'guardkids:push' } }));

    expect(invalidate).toHaveBeenCalledWith({ queryKey: ['child', 'requests'] });
    expect(invalidate).toHaveBeenCalledWith({ queryKey: ['child', 'notifications'] });
    expect(invalidate).toHaveBeenCalledWith({ queryKey: ['child', 'me'] });
  });
